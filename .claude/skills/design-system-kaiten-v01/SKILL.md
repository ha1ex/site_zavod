...
### Em-dash + короткое слово — перенос вместе (ОБЯЗАТЕЛЬНО)

Если после длинного тире (`—`) идёт **короткое слово** (1–3 буквы: `это`, `как`, `так`, `все`, `для`, `наш`, союзы/частицы и т.п.), это слово при переносе строки уходит на следующую строку **вместе с тире** — тире не остаётся в конце строки, оторванное от своего слова.

Реализация — неразрывный пробел `&nbsp;` **после** тире, между тире и коротким словом. Перед тире оставляй обычный пробел (там перенос разрешён), чтобы тире с коротким словом переезжали на новую строку вместе.

```html
<!-- ❌ тире висит в конце строки, «это» оторвано -->
<p>Kaiten — это платформа управления работой</p>

<!-- ✅ «— это» переносятся на следующую строку вместе -->
<p>Kaiten —&nbsp;это платформа управления работой</p>
```

- Применяется ко всему видимому тексту макета (заголовки, подзаголовки, тела, карточки, подписи), на всех адаптивах.
- Если слово после тире длинное — правило не нужно (обычный пробел, естественный перенос).
- Не путать с висячими предлогами выше: там предлог клеится к следующему слову; здесь короткое слово после тире клеится к **тире**.
